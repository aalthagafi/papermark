...
export const sendEmail = async ({
  ...
  const fromAddress =
    from ??
    (marketing
      ? "Marc from Khaznah <marc@ship.app.kaznah.com>"
      : system
        ? "Khaznah <system@app.kaznah.com>"
        : verify
          ? "Khaznah <system@verify.app.kaznah.com>"
          : !!scheduledAt
            ? "Marc Seitz <marc@app.kaznah.com>"
            : "Marc from Khaznah <marc@app.kaznah.com>");
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      cc: cc,
      replyTo: marketing ? "marc@app.kaznah.com" : replyTo,
      subject,
      react,
      scheduledAt,
      text: plainText,
