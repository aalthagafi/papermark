...
export function ScheduleCallModal({
  ...
  useEffect(() => {
    if (open) {
      (async function () {
        try {
          const cal = await getCalApi({ namespace: "khaznah-support" });
  ...
  return (
    ...
    >
      ...
      <div className="max-h-[540px] rounded-lg border bg-white dark:bg-gray-800">
        {calLoaded ? (
          <Cal
            namespace="khaznah-support"
            calLink={`marcseitz/khaznah-support?email=${session?.user?.email || ""}&name=${session?.user?.name || ""}`}
            style={{
              width: "100%",
              height: "540px",
              overflow: "scroll",
