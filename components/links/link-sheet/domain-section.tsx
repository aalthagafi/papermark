...
export default function DomainSection({
  ...
  const [displayValue, setDisplayValue] = useState<string>("app.kaznah.com");
  ...
  const isEditingCustomDomain =
    editLink && data.domain && data.domain !== "app.kaznah.com" ? true : false;
  ...
  const handleDomainChange = (value: string) => {
    ...
    if (isEditingCustomDomain && !canChangeCustomDomain) {
      setDisplayValue(data.domain ?? "app.kaznah.com");
      return;
    }
    ...
    if (value === "add_domain" || value === "add_dataroom_domain") {
      setModalOpen(true);
      setData({ ...data, domain: "app.kaznah.com" });
      setDisplayValue("app.kaznah.com");
      return;
    }

    // Check if this is a custom domain selection (not app.kaznah.com)
    if (value !== "app.kaznah.com") {
  ...
  useEffect(() => {
    if (domains && !editLink) {
      ...
      const domainValue = canUseCustomDomain
        ? (defaultDomain?.slug ?? "app.kaznah.com")
        : "app.kaznah.com";
  ...
  const defaultDomain = editLink
    ? (data.domain ?? "app.kaznah.com")
    : (linkType === "DOCUMENT_LINK" && canUseCustomDomainForDocument) ||
        (linkType === "DATAROOM_LINK" && canUseCustomDomainForDataroom)
      ? (domains?.find((domain) => domain.isDefault)?.slug ?? "app.kaznah.com")
      : "app.kaznah.com";
  ...
  return (
    <>
        ...
        >
          <SelectTrigger
            className={cn(
              ...
              data.domain && data.domain !== "app.kaznah.com"
                ? ""
          ...
          <SelectContent className="flex w-full rounded-md border border-input bg-white text-foreground placeholder-muted-foreground focus:border-muted-foreground focus:outline-none focus:ring-inset focus:ring-muted-foreground dark:border-gray-500 dark:bg-gray-800 focus:dark:bg-transparent sm:text-sm">
            <SelectItem value="app.kaznah.com" className="hover:bg-muted">
              app.kaznah.com
            </SelectItem>
            {linkType === "DOCUMENT_LINK" && (
              <>
        ...
        {data.domain && data.domain !== "app.kaznah.com" ? (
          <Input
            ...
            className={cn(
              ...
              data.domain && data.domain !== "app.kaznah.com" ? "flex" : "",
      ...
      {data.domain && data.domain !== "app.kaznah.com" && !isDomainVerified ? (
