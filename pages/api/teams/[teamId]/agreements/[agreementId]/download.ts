...
export default async function handle(
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      // Check if the content is a Khaznah URL
      const isKhaznahUrl =
        agreement.content.includes("app.kaznah.com/view/") ||
        agreement.content.includes("app.kaznah.com/view/");
      ...
      if (isKhaznahUrl) {
        // Extract linkId from Khaznah URL
        const urlParts = agreement.content.split("/view/");
        if (urlParts.length < 2) {
          return res.status(400).json("Invalid Khaznah URL format");
        }
        ...
        if (!link || !link.document) {
          return res
            .status(404)
            .json("Document not found for the provided Khaznah URL");
        }
